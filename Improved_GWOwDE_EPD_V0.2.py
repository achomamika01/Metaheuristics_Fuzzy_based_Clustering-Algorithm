# -*- coding: utf-8 -*-
"""
Created on Thu Dec  9 11:13:38 2021
...
#print("Now your grey wolf optimization algorithm is running:")
#maxinteration_string = input("\n\nPlease enter the MAXIMUM NUMBER OF ITERATION : ")
print("\n\nPlease wait.... Improve grey wolf optimization with DE EPD algorithm is running")
...
f_min = 0.25; f_max = 1.5; Max_gen = maxinteration; CR = 0.7
with alive_bar((pop_max+1)-pop_min) as bar:
    for bb in range(pop_min, pop_max+1):
        ...
        #print("length verifty:",len(gwo_population))
        if len(gwo_population) > 3:
            ...
            while t < maxinteration: #this part of module can be define as function to call multiple times
                ...
                temp_sol = []; temp_sol_sil=[]; de_jmeasure_center=[]
                for i in range(0, len(sorted_jmeasure_center)):
                    ...
                    X3 = delta - (A3*D_delta)
                    gwo_sol = (X1+X2+X3)/3
                    gwo_sol = gwo_sol.tolist()
                    trial_vector = np.array(gwo_sol)
                    '''gwo_membership = update_membership_matrix(gwo_sol, dataset)
                    gjm = j_measure(gwo_membership, gwo_sol)
                    temp_sol.append([gjm,gwo_sol])'''
                    iteration = t
                    # Applying mutation opeation
                    F = f_min + (f_max - f_min) * ((Max_gen - (iteration -1))/(Max_gen))
                    mutant_vector = alpha + F * (beta-delta)
                    mut_vec_list.append(mutant_vector)
                    # applying cross over operation
                    if random.random() < CR:
                        J = random.randint(0, len(dataset[0])-1)
                        #for kk in range (0, len(trial_vector)):
                        for mm in range(0, len(trial_vector)):
                            trial_vector[mm][J] = mutant_vector[mm][J]
                    else:
                        trial_vector = np.array(gwo_sol)
                    # applying selection operation
                    gwo_membership = update_membership_matrix(np.array(gwo_sol), dataset)
                    gjm = j_measure(gwo_membership, gwo_sol)
                    gwo_membership_trial = update_membership_matrix(trial_vector, dataset)
                    gjm_trial = j_measure(gwo_membership, trial_vector)
                    
                    if gjm < gjm_trial:
                        temp_sol.append([gjm,gwo_sol])
                    else:
                        temp_sol.append([gjm_trial,trial_vector])
                    ...
                    #temp_sol_sil.append([gjm, gwo_silhouette_avg, gwo_sol])
                ...
                # applying EPD operator
                alpha = np.array(sorted_jmeasure_center[0][1])
                beta = np.array(sorted_jmeasure_center[1][1])
                delta = np.array(sorted_jmeasure_center[2][1])
                ub = np.amax(dataset); lb = np.amin(dataset) 
                position_alpha_1 = alpha + (ub - lb * random.random() + lb)
                position_alpha_2 = alpha - (ub - lb * random.random() + lb)
                position_beta_1 = beta + (ub - lb * random.random() + lb)
                position_beta_2 = beta - (ub - lb * random.random() + lb)
                position_delta_1 = delta + (ub - lb * random.random() + lb)
                position_delta_2 = delta - (ub - lb * random.random() + lb)
                #any_position =  (ub - lb * random.random() + lb)
                probable_position = [position_alpha_1 ,position_alpha_2,  position_beta_1, position_beta_2,
                                    position_delta_1,  position_delta_2]
                half = int(len(sorted_jmeasure_center)/2)
                for ii in range(0, half):
                    de_jmeasure_center.append(sorted_jmeasure_center[ii])
                for ll in range(half, len(sorted_jmeasure_center)):
                    de_jmeasure_center.append(probable_position[random.randint(0, len(probable_position)-1)])
                sorted_jmeasure_center = np.array(de_jmeasure_center)
                gwo_cluster_label = labeler(sorted_jmeasure_center[0][1], dataset)
                try:
                    gwo_silhouette_avg = silhouette_score(dataset, gwo_cluster_label)
                                      
                except:
                    gwo_silhouette_avg = 0 
                gxi = xie_beni_index(sorted_jmeasure_center[0][1],sorted_jmeasure_center[0][0])
                j_s_center.append([bb,sorted_jmeasure_center[0][0],gxi,gwo_silhouette_avg,sorted_jmeasure_center[0][1]])    
                t =t+1
                #print("T - ",t)
            #best_sil_clus.append([best_bb, best_tt, best_ii, max_sil, best_clus])
...
'''
Need to refer tomorrow below url
...
import pandas as pd
pd.DataFrame(j_s_center).to_csv('Skin_Output_JM_XI_Sihoute_Center_220.csv')   
...
checking = input("Please press ENTER key :")
for aa in silhoutte_index:
    ...
    try:
        ...
        db_score = 1
    if silhouette_avg !=0:
        print("\n\nFor n_clusters =",n_clusters,"The Calinski-Harabasz_score is :",ch_score)
        ...
        plt.suptitle(
            "Silhouette analysis for Improved GWO with DE EPD clustering on sample data with n_clusters = %d"
            % n_clusters,
            fontsize=14,
            fontweight="bold",
        )
